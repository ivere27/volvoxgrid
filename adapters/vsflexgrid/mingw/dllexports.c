...
extern int vfg_object_registry_count(void);
...
static void delete_reg_tree(HKEY hRoot, const char *subkey) {
    ...
    RegDeleteKeyA(hRoot, buf);
    snprintf(buf, sizeof(buf), "%s\\CLSID", subkey);
    RegDeleteKeyA(hRoot, buf);
    snprintf(buf, sizeof(buf), "%s\\CurVer", subkey);
    RegDeleteKeyA(hRoot, buf);
...
STDAPI DllCanUnloadNow(void) {
    return (g_cLocks == 0 && vfg_object_registry_count() == 0) ? S_OK : S_FALSE;
}
