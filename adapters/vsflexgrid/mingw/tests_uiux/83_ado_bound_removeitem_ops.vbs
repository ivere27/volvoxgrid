' Test 83: Observe recordset and grid state after RemoveItem on a bound grid.
...
If Err.Number = 0 Then
    ...
    rs.AddNew: rs("ITEM_CODE") = "R-01": rs("ITEM_NAME") = "Rotor": rs.Update
    rs.AddNew: rs("ITEM_CODE") = "R-02": rs("ITEM_NAME") = "Seal": rs.Update
    rs.AddNew: rs("ITEM_CODE") = "R-03": rs("ITEM_NAME") = "Bracket": rs.Update
    ...
    fg.TextMatrix(0, 0) = "RC=" & SafeCount(rs)
    fg.TextMatrix(0, 1) = "CURR=" & SafeField(rs, "ITEM_CODE")
Else
    Call SetupFallback("RemoveItem")
End If
